...
async function fetchAndCacheOrgs(forceRefresh: boolean = false): Promise<any[]> {
    ...
    if (!forceRefresh) {
        const cachedOrgs = globalContext.globalState.get<any[]>("salesforceOrgs");
        if (cachedOrgs) {
...
const openNewTabDisposable = vscode.commands.registerCommand("organetto.openNewTab", async () => {
    ...
    const lastOpenedTimes = globalContext.globalState.get<Record<string, string>>("orgLastOpenedTimes") || {};
                        ...
                        async (progress) => {
                            try {
                                const refreshLastOpenedTimes =
                                    globalContext.globalState.get<Record<string, string>>("orgLastOpenedTimes") || {};
                    ...
                    await vscode.window.withProgress(
                        ...
                        async (progress) => {
                            try {
                                ...
                                await globalContext.globalState.update("orgLastOpenedTimes", lastOpenedTimes);
                        ...
                        await vscode.window.withProgress(
                            ...
                            async (progress) => {
                                try {
                                    ...
                                    await globalContext.globalState.update("orgLastOpenedTimes", logoutLastOpenedTimes);
                                    ...
                                    await globalContext.globalState.update("salesforceOrgs", updatedOrgs);
...
function getWebviewContent(
    ...
    for (const dir of possiblePaths) {
        const testPath = path.join(globalContext.extensionPath, dir, "webview", "template.html");
        if (fs.existsSync(testPath)) {
            templatePath = testPath;
            stylesPath = path.join(globalContext.extensionPath, dir, "webview", "styles.css");
        ...
        async (progress) => {
            ...
            try {
                ...
                if (newOrg) {
                    ...
                    const filteredOrgs = cachedOrgs.filter((org) => (org.alias || org.username) !== alias);

                    // Add the new org to the beginning of the list
                    filteredOrgs.unshift(newOrg);
                    await globalContext.globalState.update("salesforceOrgs", filteredOrgs);
                ...
                vscode.window.showErrorMessage(`Authentication completed, but failed to fetch org details: ${error}`);
